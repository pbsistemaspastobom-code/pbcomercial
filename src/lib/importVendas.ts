...
export interface ResultadoImport {
  ...
  naoReconhecidos: { nome: string; valor: number }[];
...
export interface Matcher { casar: (nomeRaw: string) => { codigo: string; combinado: boolean } | null; nomePorCodigo: Record<string, string>; }
export function criarMatcher(vendedores: Vend[]): Matcher {
  ...
  const casar = (nomeRaw: string): { codigo: string; combinado: boolean } | null => {
...
function processarGrid(grid: unknown[][], m: Matcher): AbaResultado | null {
  ...
  const naoRec: Record<string, number> = {};
  let carriedName = "";
  for (let i = det.dataStart; i < grid.length; i++) {
    ...
    if (mm) {
      ...
      naoRec[nlimpo] = (naoRec[nlimpo] ?? 0) + valor;
    }
  }
...
export async function importarPlanilha(file: File, vendedores: Vend[] = VENDEDORES): Promise<ResultadoImport> {
  ...
  return {
    ...
    naoReconhecidos: Object.entries(melhor.naoRec).map(([nome, valor]) => ({ nome, valor })),
    diagnostico,
