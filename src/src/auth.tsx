  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#f4f6f3] p-4">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl border border-[#e6e9e3] overflow-hidden">
        <div className="bg-[#0F2E1E] px-8 py-8 text-center">
        ...
        <div className="p-8 space-y-4">
          <div>
            ...
            <input value={u} onChange={(e) => setU(e.target.value)} onKeyDown={(e) => e.key === "Enter" && entrar()}
              className="mt-1 w-full h-11 rounded-lg border border-[#d6dbd2] px-3 focus:outline-none focus:ring-2 focus:ring-[#3A7D2E]/40" autoFocus />
          </div>
          <div>
            ...
            <input type="password" value={s} onChange={(e) => setS(e.target.value)} onKeyDown={(e) => e.key === "Enter" && entrar()}
              className="mt-1 w-full h-11 rounded-lg border border-[#d6dbd2] px-3 focus:outline-none focus:ring-2 focus:ring-[#3A7D2E]/40" />
          </div>
          {erro && <div className="text-sm text-[#9A3B32] bg-[#FBF3D6] rounded-lg px-3 py-2">{erro}</div>}
          <button onClick={entrar} disabled={carregando}
            className="w-full h-11 rounded-lg bg-[#3A7D2E] hover:bg-[#245c43] text-white font-semibold disabled:opacity-60">
