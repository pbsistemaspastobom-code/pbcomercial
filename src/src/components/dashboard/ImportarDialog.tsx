...
export const ImportarDialog = React.memo(function ImportarDialog({ resultado, vendedores, ano, mesInicial, contextoLabel, onConfirmar, onCancelar, gravando }: Props) {
      ...
      <DialogContent className="max-w-2xl max-h-[88vh] overflow-hidden flex flex-col">
        ...
        {resultado && (
          <div className="overflow-auto flex-1 space-y-4 text-sm">
            <div className="flex items-center gap-2 bg-pasto-claro/60 rounded-lg px-3 py-2">
              {contextoLabel ? (
                <><span className="text-sm font-semibold text-primary">{contextoLabel}</span><span className="text-xs text-muted-foreground">Os valores acumulados serão gravados neste trimestre.</span></>
              ) : (
                <>
            ...
            {resultado.combinados.length > 0 && (
              <div>
                <div className="font-semibold text-[#9A7A00] mb-1">Combinados ({resultado.combinados.length})</div>
            ...
            {resultado.naoReconhecidos.length > 0 && (
              <div>
                <div className="font-semibold text-[#9A3B32] mb-1">Novos na planilha ({resultado.naoReconhecidos.length}) — vincule a um vendedor do sistema</div>
        ...
        <DialogFooter className="border-t pt-3">
          ...
          <Button onClick={() => onConfirmar(assign, mes)} disabled={gravando}>
            {gravando ? "Gravando..." : contextoLabel ? "Confirmar e gravar" : `Confirmar e gravar em ${MESES_LONGO[mes - 1]}`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
