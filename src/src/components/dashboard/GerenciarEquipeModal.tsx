              ...
              {lista.map((v) => (
                <tr key={v.codigo} className="border-t border-[#eef1eb]">
                  ...
                  <td className="py-2 text-center">
                    <button onClick={() => pedirExcluir(v)} disabled={checando} title="Excluir" className="text-[#9A3B32] hover:opacity-70 disabled:opacity-40">
        ...
        {confirmar && (
          <div className="absolute inset-0 z-20 flex items-center justify-center p-4 bg-black/40 rounded-2xl">
            <div className="bg-white rounded-xl p-5 max-w-md w-full shadow-xl">
              <div className="flex items-center gap-2 text-[#9A3B32] font-bold mb-2"><AlertTriangle className="w-5 h-5" /> Excluir vendedor</div>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button className="bg-[#9A3B32] hover:bg-[#7d2f28]" onClick={confirmarExclusao} disabled={excluindo}>{excluindo ? "Excluindo..." : "Excluir"}</Button>
              </div>
            </div>
          </div>
        )}
